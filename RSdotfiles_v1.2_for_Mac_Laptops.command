...
display_header() {
 ...
 |   /\__ \ / _` / _ \  _|  _| | / -_|_-<
 |_|_\|___/ \__,_\___/\__|_| |_|_\___/__/
EOF
    echo " ${BL}Created by Ryan Summer${NC} | ${BL}For macOS 12-26${NC}"
    echo
    echo "${GR}Version 1.2${NC} ${BO}${GY}(For MacBook Pro/Laptops)${NC}"
    echo "${BL}Updated 09/14/2025${NC}"
    echo
    echo "${YE}Note:${GR}"
    echo "- For Intel, just comment out the last 3 lines under 'Apple Intelligence'."
    echo "- For VM's using UTM, enabling/disabling Time Machine Auto Backup does not work."
    echo "  If not using UTM & you want to disable Time Machine Auto Backup, uncomment the"
    echo "  last line under ${NC}${BO}'💾 [Disks]'${NC}${GR}."
    echo "- To see all available options not shown here or to see what works or doesn't"
    echo "  work in macOS Tahoe, please check out the MacOS Preferences menu option"
    echo "  in my free tool OneCommand here:${NC} ${BL}https://shop.ryansummer.com/p/onecommand/${NC}"
    echo
}
...
echo "🚪 ${GR}Quiting System Preferences/Settings...${NC}"
...
echo "🚪 ${GR}Quiting System Preferences/Settings...again${NC}"
...
# If on Tahoe, feel free to enable the commands below
...
echo "⚓️ [Dock]"
defaults delete com.apple.dock persistent-apps -array    # WARNING: Removes all default app icons from the Dock."
